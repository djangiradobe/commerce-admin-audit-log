...
function buildFilter(params) {
    const f = {};
    if (params.scope)
        f.scope = String(params.scope);
    if (params.scopeId)
        f.scope_id = String(params.scopeId);
    if (params.actor)
        f.changedBy = String(params.actor);
    if (params.action && ['create', 'update', 'delete'].includes(params.action)) {
        f.action = params.action;
    }
    if (params.since)
        f.changedAt = { ...(f.changedAt || {}), $gte: String(params.since) };
    if (params.until)
        f.changedAt = { ...(f.changedAt || {}), $lte: String(params.until) };
    return f;
}
    ...
    try {
        ...
        try {
            ...
            if (!/exist|already|duplicate/i.test(m))
                logger.warn(`ensureCollection: ${m}`);
        }
