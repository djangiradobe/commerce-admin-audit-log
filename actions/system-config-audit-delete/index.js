...
function normalizeKeys(params) {
    ...
    return params.keys
        ...
        const f = {};
        if (k.changedAt != null)
            f.changedAt = String(k.changedAt);
        if (k.path != null)
            f.path = String(k.path);
        if (k.scope != null)
            f.scope = String(k.scope);
        if (k.scope_id != null)
            f.scope_id = String(k.scope_id);
        if (k.action != null)
            f.action = String(k.action);
        return f;
    });
}
async function main(params) {
    ...
    if (requested === 0)
        return { statusCode: 400, body: { ok: false, error: 'Provide keys (or ids) to delete.' } };
    let handle;
    try {
    ...
    try {
        ...
        for (const key of keys) {
            try {
                deleted += countOf(await col.deleteMany(key));
            }
            catch (_) {
                try {
                    deleted += countOf(await col.deleteOne(key));
                }
                catch (_) { }
            }
        }
        ...
        if (keys.length === 0 && ids.length) {
            try {
                deleted += countOf(await col.deleteMany({ _id: { $in: ids } }));
            }
            catch (_) {
                for (const id of ids) {
                    try {
                        deleted += countOf(await col.deleteOne({ _id: id }));
                    }
                    catch (_) { }
                }
            }
